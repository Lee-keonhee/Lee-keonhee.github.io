...
| 한계점   | 고정된 기울기가 항상 최적의 성능을 보장하지 못함                                |
...
## 2. 학습률(learning rate)
학습률은 옵티마이저가 손실 함수의 최소값을 찾아가는 과정에서 각 반복(iteration)마다 가중치를 업데이트하는 스텝의 크기를 결정하는 매개변수입니다 . 이 값이 너무 높을 경우 최적 지점을 지나쳐 발산할 위험이 있으며, 너무 낮을 경우 수렴 속도가 느려지거나 지역 최적점에 갇힐 수 있습니다.

## 3. epoch
에포크는 전체 학습 데이터셋이 신경망 모델을 한 번 완전히 통과한 횟수를 의미합니다. 즉, 모든 훈련 데이터가 한 번씩 모델 학습에 사용되는 단위를 나타냅니다. 에포크 수가 너무 많으면 모델이 훈련 데이터에 과적합될 가능성이 증가합니다.  

## 4. batch_size
배치 사이즈는 한 번의 가중치 업데이트를 위해 사용되는 훈련 샘플의 개수입니다. 전체 데이터셋을 여러 개의 작은 그룹으로 나누었을 때, 한 소그룹에 포함되는 데이터 수를 의미하기도 합니다. 배치 사이즈는 모델 학습 속도와 메모리 사용량에 영향을 미치며, 일반적으로 크기가 클수록 한 에포크당 학습 속도가 빠르지만, 일반화 성능에는 복합적인 영향을 줄 수 있습니다.  
